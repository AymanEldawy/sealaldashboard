  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className='pt-6 px-4 pb-4 bg-white rounded-b-xl'>
      ...
      <div className='flex gap-4 mt-4 items-end'>
        <CustomSelectField containerClassName='text-sm max-w-full flex-1 !mb-0 rounded-md' label={fetchWord('Category', lang)} placeholder={fetchWord('Category', lang)} {...register("category", { required: true, maxLength: 20 })} />
        <CustomSelectField containerClassName='text-sm max-w-full flex-1 !mb-0 rounded-md' label={fetchWord('Brand', lang)} placeholder={fetchWord('Brand', lang)} {...register("brand", { required: true, maxLength: 20 })} />
