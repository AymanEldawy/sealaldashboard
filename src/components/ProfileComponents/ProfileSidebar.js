...
export const ProfileSidebar = ({ containerClassName, username, role, activeTab, setActiveTab, changeUserRole }) => {
  ...
  return (
    <aside className={`sticky top-0 h-[100dvh] flex flex-col w-[286px] border-2 ${containerClassName}`}>
      ...
      />
      <button onClick={handleLogout} className="capitalize ltr:pl-8 rtl:pr-8 py-4 font-semibold border-b relative whitespace-nowrap w-full ltr:text-left rtl:text-right">{fetchWord('logout', lang)}</button>
