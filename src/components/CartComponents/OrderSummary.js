  ...
  return (
    <div className='bg-[#FAFAFA] p-8'>
      ...
      <div className='flex border-b border-[#CFCFCF] pb-2 mt-4 justify-between'>
        <h4 className='capitalize font-medium flex-1'>{fetchWord('Product_name', lang)}</h4>
        <h4 className='capitalize font-medium text-center w-[80px]'>{fetchWord('quantity', lang)}</h4>
        <h4 className='capitalize font-medium text-center w-[70px]'>{fetchWord('price', lang)}</h4>
      </div>
      {
        cart?.map(item => (
