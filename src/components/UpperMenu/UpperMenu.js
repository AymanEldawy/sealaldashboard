  ...
  return (
    <div className=" bg-secondary-dark text-white py-1">
      <div className="text-xs flex justify-between flex-wrap gap-4 items-center min-h-[35px] container">
        <div className="flex items-center gap-4 justify-between lg:justify-start w-full lg:w-fit">
          <Link href="">{fetchWord('Vendor_Panel', lang)}</Link>
          <Link href="">{fetchWord('Seller_Information_Center', lang)}</Link>
        </div>
        <div className="hidden lg:flex relative gap-3 md:gap-6 items-center overflow-auto">
          <Link href="/notifications" className="flex gap-1 items-center whitespace-nowrap flex-col sm:flex-row font-medium max-[450px]:flex-col "><BellIcon className="w-5 h-5 text-white fill-white" /> {fetchWord('My_Announcements', lang)}</Link>
